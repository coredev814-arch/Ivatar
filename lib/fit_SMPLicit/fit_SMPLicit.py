...
from SMPLicit.mesh_utils import SimpleMesh, mesh_to_sdf
import time
import tqdm
...
# Garment catalog: stores fitted latent codes for reuse
GARMENT_CATALOG_PATH = 'garment_catalog.json'
CLOTH_LABEL_NAMES = {
    5: 'tshirt', 7: 'coat', 9: 'pants', 12: 'skirt', 18: 'shoe', 2: 'hair'
}
CLOTH_LABEL_TO_MODEL_ID = {
    5: 0, 7: 0, 9: 1, 12: 2, 18: 4, 2: 3
}
garment_catalog = []
    ...
    for index_fitting in range(len(smpl_prediction)):
        ...
        for cloth_optimization_index in _opt.labels:
            ...
            if _opt.repose:
                ...
                v_inference = v
            smpl_mesh = SimpleMesh.from_tensors(v_inference[0].cuda(), smpl_faces.cuda())
            smpl_mesh_sdf = mesh_to_sdf(smpl_mesh)
            ...
            # Save fitted latent codes to garment catalog
            style_np = style.cpu().data.numpy().flatten().tolist()
            shape_np = shape.cpu().data.numpy().flatten().tolist()
            z_full = shape_np + style_np  # z_cut + z_style
            garment_entry = {
                'id': f"{_file}_person{index_fitting}_{CLOTH_LABEL_NAMES.get(cloth_optimization_index, str(cloth_optimization_index))}",
                'source_image': _file,
                'cloth_label': int(cloth_optimization_index),
                'cloth_name': CLOTH_LABEL_NAMES.get(cloth_optimization_index, 'unknown'),
                'model_id': CLOTH_LABEL_TO_MODEL_ID.get(cloth_optimization_index, 0),
                'z_cut': shape_np,
                'z_style': style_np,
                'z_full': z_full,
                'n_z_cut': len(shape_np),
                'n_z_style': len(style_np),
            }
            garment_catalog.append(garment_entry)
...
# Save garment catalog with all fitted latent codes
if garment_catalog:
    with open(GARMENT_CATALOG_PATH, 'w') as f:
        json.dump(garment_catalog, f, indent=2)
    print(f"Saved {len(garment_catalog)} garments to {GARMENT_CATALOG_PATH}")

...
from .mesh_utils import SimpleMesh
...
class SMPLicit(nn.Module):
    def __init__(self):
        ...
        self.SMPL_Layer = SMPL(_opt.path_SMPL, obj_saveable=True).to(self.device)
        self.smpl_faces = self.SMPL_Layer.faces

        self.Astar_pose = torch.zeros(1, 72).to(self.device)
    ...
    def reconstruct(self, model_ids=[0], Zs=[np.zeros(18)], pose=np.zeros(72), beta=np.zeros(10)):
        ...
        pose = pose.reshape(1,-1).to(device)
        beta = beta.reshape(1,-1).to(device)
        ...
        inference_mesh = SimpleMesh.from_tensors(unposed_smpl[0],
                                        torch.LongTensor(self.smpl_faces).to(device))
        inference_lowerbody = SimpleMesh.from_tensors(Astar_smpl,
                                        torch.LongTensor(self.smpl_faces).to(device))
    ...
    def pose_mesh(self, mesh, pose, J, v):
        ...
        for i in range(iters):
            in_verts = torch.FloatTensor(mesh.vertices[i*step:(i+1)*step]).to(self.device).unsqueeze(0)
    ...
    def pose_mesh_lowerbody(self, mesh, pose, beta, J, v):
        ...
        if len(mesh.vertices)%step != 0:
            iters += 1
        for i in range(iters):
            in_verts = torch.FloatTensor(mesh.vertices[i*step:(i+1)*step]).to(self.device)

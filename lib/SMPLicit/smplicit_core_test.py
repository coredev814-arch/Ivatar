...
class Model():
    ...
    def reconstruct(self, z, smpl_trianglemesh):
        ...
        if not torch.is_tensor(z_cut):
            z_cut = torch.FloatTensor(z_cut).to(self.device)
        if not torch.is_tensor(z_style):
            z_style = torch.FloatTensor(z_style).to(self.device)

        z_cut = z_cut.to(self.device).reshape(1, -1)
        z_style = z_style.to(self.device).reshape(1, -1)

        smpl_points = smpl_trianglemesh.vertices
        smpl_points = smpl_points[self.clusters[self.num_clusters]]

        def eval_func(points):
            points = torch.FloatTensor(points).transpose(1, 0).to(self.device)
    ...
    def load(self):
        ...
        self._G.load_state_dict(torch.load(self.filename, map_location=device))

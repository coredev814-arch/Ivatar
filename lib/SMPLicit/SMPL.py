...
def _load_smpl_model(model_path):
    """Load SMPL model from either .pkl or .txt (JSON) format."""
    if model_path.endswith('.pkl'):
        return _load_pkl(model_path)
    else:
...
def _load_pkl(path):
    """Load a SMPL .pkl file, handling chumpy dependencies gracefully."""
    # Mock chumpy modules so pickle can deserialize without the real package
    _mock_modules = {}
    for mod_name in ['chumpy', 'chumpy.ch', 'chumpy.ch_ops',
                     'chumpy.utils', 'chumpy.linalg', 'chumpy.reordering']:
        if mod_name not in sys.modules:
            m = types.ModuleType(mod_name)
            sys.modules[mod_name] = m
            _mock_modules[mod_name] = m

    class _FakeCh(np.ndarray):
    ...
    # Convert sparse matrices and chumpy arrays to plain numpy
    for key in data:
        if sparse.issparse(data[key]):
            data[key] = np.array(data[key].todense())
        elif isinstance(data[key], _FakeCh):
...
class SMPL(nn.Module):
    def __init__(self, model_path, joint_type = 'cocoplus', obj_saveable = False):
        ...
        # Support both .pkl and .txt (JSON) model files
        model = _load_smpl_model(model_path)
...
if __name__ == '__main__':
    ...
    pose= np.array([
            ...
            3.55101633e-03,  -2.68924050e-02,   4.93676625e-02],dtype = np.float64)
